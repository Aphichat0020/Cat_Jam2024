...
namespace BlazeAISpace
{
    ...
    public class GoingToCoverBehaviour : MonoBehaviour
    {
        ...
        [Min(-1), Tooltip("This is the min and max height values to search for a cover point. This is used to prevent the AI from going to a cover point that's on a higher or lower level in a building for example. Don't play with this unless you have problems where the AI is choosing a cover point too high or too low. The X value is the minimum while the Y value is the maximum. It's calculated using normalized Vector3.Dot() from AI to cover point.")]
        public Vector2 minAndMaxCoverPointHeight = new Vector2(-0.45f, 0.45f);
        ...
        {   
            ...
            for (int i=0; i<hits; i++) {
                ...
                float heightProjection = Vector3.Dot(transform.up, directionToCover);
                ...
                if (distance + 2 >= coverShooterBehaviour.distanceFromEnemy || distance > blaze.vision.visionDuringAttackState.sightRange ||
                    ...
                    heightProjection <= minAndMaxCoverPointHeight.x || heightProjection >= minAndMaxCoverPointHeight.y ||
